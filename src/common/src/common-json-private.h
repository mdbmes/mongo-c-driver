...
mcommon_json_append_value_int32 (mcommon_string_append_t *append, int32_t value, bson_json_mode_t mode)
{
   return mode == BSON_JSON_MODE_CANONICAL
             ? mcommon_string_appendf (append, "{ \"$numberInt\" : \"%" PRId32 "\" }", value)
             : mcommon_string_appendf (append, "%" PRId32, value);
}
...
{
   return mode == BSON_JSON_MODE_CANONICAL
             ? mcommon_string_appendf (append, "{ \"$numberLong\" : \"%" PRId64 "\" }", value)
             : mcommon_string_appendf (append, "%" PRId64, value);
}
...
/**
 * @brief Append the JSON serialization of an ObjectId value
 * @param append A bounded string append, initialized with mcommon_string_append_init()
 ...
 * @param max_depth Maximum allowed number of document/array nesting levels below this one
 * @returns true on success, false if this 'append' has exceeded its max length or we encountered invalid BSON.
 *
