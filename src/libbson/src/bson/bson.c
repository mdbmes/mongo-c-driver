 ...
 *
 * Parameters:
 *       @append: Append operation, such as initialized by mcommon_string_append_init
...
static BSON_INLINE void
_bson_append_regex_options_sorted (mcommon_string_append_t *append, const char *options)
{
   const char *c;

   for (c = BSON_REGEX_OPTIONS_SORTED; *c; c++) {
      if (strchr (options, *c)) {
         mcommon_string_append_bytes (append, c, 1);
      }
   }
}
   ...
   bson_t *bson, const char *key, int key_length, const char *regex, int regex_length, const char *options)
{
   ...
   _bson_append_regex_options_sorted (&options_sorted_append, options);
   if (!mcommon_string_append_status (&options_sorted_append)) {
      goto append_failure;
   }
