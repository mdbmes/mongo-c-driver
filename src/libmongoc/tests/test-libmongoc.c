...
static int suppress_structured_logs_counter_atomic;
...
{
   int c = mcommon_atomic_int_fetch (&suppress_structured_logs_counter_atomic, mcommon_memory_order_seq_cst);
   BSON_ASSERT (c >= 0);
   return c > 0;
}

// Ignore logs generated by test-internal operations. Can be nested. Structured logs only; see capturing_logs too.
void
test_begin_suppressing_structured_logs (void)
{
   mcommon_atomic_int_fetch_add (&suppress_structured_logs_counter_atomic, 1, mcommon_memory_order_seq_cst);
}
...
{
   int previous =
      mcommon_atomic_int_fetch_sub (&suppress_structured_logs_counter_atomic, 1, mcommon_memory_order_seq_cst);
   BSON_ASSERT (previous > 0);
}
...
test_libmongoc_destroy (TestSuite *suite)
{
   ...
   BSON_ASSERT (!test_is_suppressing_structured_logs ());
   mongoc_cleanup ();
}
